...
web/                            # Astro + Tailwind landing page (served at https://datalake.example.com/)
├── src/
│   ├── layouts/Base.astro
│   ├── pages/index.astro
...
`web/` is a static Astro site served at the apex `https://datalake.example.com/`. It's built inside `web.Dockerfile` (Node builder → Caddy runtime) and wired into `docker-compose.prod.yml` as the `web` service. The edge Caddy proxies `/api/*` to the FastAPI container (prefix-stripped) and everything else to `web:80`.
