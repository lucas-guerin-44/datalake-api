...
| `<DOMAIN>` | `datalake.example.com` | or leave Caddyfile on `:80` for IP-only |
...
- **Domain**: keep the Caddyfile as-is (it already has `datalake.example.com`
  with your email for Let's Encrypt).
- **IP-only (temporary)**: edit `Caddyfile`, replace the `datalake.example.com {`
...
with get_db_context() as db:
    if not get_user_by_username(db, "lucas"):
        create_user(db, "lucas", "you@example.com", get_password_hash(secrets.token_urlsafe(32)))
        print("created")
    else:
        print("exists")
EOF
